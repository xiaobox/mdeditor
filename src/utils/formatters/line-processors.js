...
export class CodeBlockProcessor extends LineProcessor {
  ...
  process(line, trimmedLine, context) {
    const { inCodeBlock, codeBlockContent, codeBlockLanguage, currentTheme, codeTheme } = context;
    ...
    if (!inCodeBlock) {
      ...
      // 结束代码块
      const result = formatCodeBlock(codeBlockContent, codeBlockLanguage, currentTheme, codeTheme);
      return {
        result,
        shouldContinue: true,
        updateContext: {
...
export class BlockquoteProcessor extends LineProcessor {
  ...
  process(line, trimmedLine, context) {
    const { inBlockquote, blockquoteContent } = context;
    ...
    if (!inBlockquote) {
      return {
        ...
        updateContext: {
          inBlockquote: true,
          blockquoteContent: [processedLine]
        }
      };
    } else {
      return {
        ...
        shouldContinue: true,
        updateContext: {
          blockquoteContent: [...blockquoteContent, processedLine]
        }
      };
    }
  }
